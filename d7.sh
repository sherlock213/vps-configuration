...
echo "clear" >> .bashrc
echo 'echo -e "Welcome to $HOSTNAME server dengan IP: $IP :v"' >> .bashrc
...
wget -O user-baru "https://raw.githubusercontent.com/sherlock213/vps-configuration/master/userbaru.sh"
wget -O user-trial "https://raw.githubusercontent.com/sherlock213/vps-configuration/master/trial.sh"
wget -O user-hapus "https://raw.githubusercontent.com/sherlock213/vps-configuration/master/hapususer.sh"
wget -O user-cek "https://raw.githubusercontent.com/sherlock213/vps-configuration/master/cek.sh"
wget -O member "https://raw.githubusercontent.com/sherlock213/vps-configuration/master/member.sh"
wget -O revis "https://raw.githubusercontent.com/sherlock213/vps-configuration/master/resv.sh"
...
echo "------"  | tee -a log-install.txt
echo "  Daftar Perintah:"  | tee -a log-install.txt
echo "* menu        : menampilkan daftar perintah"  | tee -a log-install.txt
echo "* user-baru   : membuat akun SSH & OpenVPN"  | tee -a log-install.txt
echo "* user-trial  : membuat akun trial"  | tee -a log-install.txt
echo "* user-hapus  : menghapus akun SSH & OpenVPN"  | tee -a log-install.txt
echo "* user-cek    : cek user login"  | tee -a log-install.txt
echo "* member      : daftar member SSH & OpenVPN"  | tee -a log-install.txt
echo "* revis       : restart service dropbear, webmin"  | tee -a log-install.txt
echo "                squid3, OpenVPN dan SSH"  | tee -a log-install.txt
echo "* reboot      : reboot VPS"  | tee -a log-install.txt
echo "* tes-speed   : speedtest VPS"  | tee -a log-install.txt
echo "* info        : menampilkan informasi sistem"  | tee -a log-install.txt
echo "* about       : info script auto install"  | tee -a log-install.txt
echo "* exit        : keluar dari Putty/Connecbot/"  | tee -a log-install.txt
echo "                JuiceSSH"  | tee -a log-install.txt
echo ""  | tee -a log-install.txt
...
echo ""  | tee -a log-install.txt
echo "Original script by :"  | tee -a log-install.txt
echo "* Fornesia"  | tee -a log-install.txt
echo "* Rzengineer"  | tee -a log-install.txt
echo "* Fawzya"  | tee -a log-install.txt
echo "Mod by Bustami Arifin"  | tee -a log-install.txt
echo "maapkan aku udh mod tnp bilang-bilang :v"  | tee -a log-install.txt
echo "Thanks buat kalian semua :'v"  | tee -a log-install.txt
echo ""  | tee -a log-install.txt
echo "Mod again by BAT :v"  | tee -a log-install.txt
echo ""  | tee -a log-install.txt
